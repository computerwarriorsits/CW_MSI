...
''ACCEPTS 2 PARAMETERS , OPTIONAL 2 PARAMETERS
''OPTIONAL PARAMETER 'STRUSR' ; USERNAME FOR CLOUD USER TO BE CREATED
''OPTIONAL PARAMETER 'STRPWD' ; PASSWORD FOR CLOUD USER TO BE CREATED
	...
	if objXML.load("https://github.com/computerwarriorsits/scripts/raw/master/version.xml") then
        ...
        objLOG.write vbnewline & now & vbtab & " - CloudClient :  " & strVER & " : GitHub : " & objSCR.text & vbnewline
				if (cint(objSCR.text) > cint(strVER)) then
					...
					call FILEDL("https://raw.githubusercontent.com/computerwarriorsits/scripts/master/CloudClient.vbs", wscript.scriptname)
					''RUN LATEST VERSION
					if (wscript.arguments.count > 0) then             ''ARGUMENTS WERE PASSED
						for x = 0 to (wscript.arguments.count - 1)

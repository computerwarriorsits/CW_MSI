...
''REQUIRED PARAMETER : 'STRDMN' , STRING TO SET DOMAIN - NO LONGER PASSED
...
''PREPARE LOGFILE
if (objFSO.fileexists("C:\temp\MSI_REPROBE_KEY")) then      ''LOGFILE EXISTS
  objFSO.deletefile "C:\temp\MSI_REPROBE_KEY", true
  set objLOG = objFSO.createtextfile("C:\temp\MSI_REPROBE_KEY")
  objLOG.close
  set objLOG = objFSO.opentextfile("C:\temp\MSI_REPROBE_KEY", 8)
...
if (errRET = 0) then                                        ''ARGUMENTS PASSED , CONTINUE SCRIPT
	objOUT.write vbnewline & vbnewline & now & vbtab & " - EXECUTING : MSI_REPROBE_KEY"
  ...
  intRET = objWSH.run ("cmd.exe /C " & chr(34) & "cscript.exe " & chr(34) & "C:\IT\Scripts\chkAU.vbs" & chr(34) & " " & _
    ...
    chr(34) & strKEY & "|" & strPRB & "|" & strDMN & "\" & strUSR & "|" & strPWD & "|" & strSVR & chr(34) & chr(34), 0, true)
  ...
  if ((intRET = 4) or (intRET = 10) or (intRET = 11) or (intRET = 1) or (intRET = 2147221505) or (intRET = 2147221517)) then
    objOUT.write vbnewline & now & vbtab & vbtab & " - NO UPDATE FOUND : MSI_REPROBE_KEY : " & strVER
    objLOG.write vbnewline & now & vbtab & vbtab & " - NO UPDATE FOUND : MSI_REPROBE_KEY : " & strVER
    ...
    call FILEDL("https://raw.githubusercontent.com/computerwarriorsits/CW_MSI/master/Windows%20Software%20Probe.msi", "C:\IT", "windows software probe.msi")
    if (errRET <> 0) then
      call LOGERR(4)
    end if
    ...
    select case lcase(strPRB)
      ''LOCAL ONLY
      case "local_windows"
        strRCMD = "msiexec /i " & chr(34) & "c:\IT\windows software probe.msi" & chr(34) & " /qn AGENTACTIVATIONKEY=" & strKEY & _
...
sub CLEANUP()                                               ''SCRIPT CLEANUP
  ...
  objOUT.write vbnewline & vbnewline & now & " - MSI_REPROBE_KEY COMPLETE" & vbnewline
  objLOG.write vbnewline & vbnewline & now & " - MSI_REPROBE_KEY COMPLETE" & vbnewline
  objLOG.close
  ''EMPTY OBJECTS
  set objLOG = nothing
  set objFSO = nothing

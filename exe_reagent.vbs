...
if (errRET = 0) then                                        ''ARGUMENTS PASSED, CONTINUE SCRIPT
  ...
  if ((intRET = 4) or (intRET = 10) or (intRET = 11) or (intRET = 1) or (intRET = 2147221505) or (intRET = 2147221517)) then
    ...
    call FILEDL("https://raw.githubusercontent.com/computerwarriorsits/CW_MSI/master/WindowsAgentSetup.exe", "C:\IT", strCID & "WindowsAgentSetup.exe")
    if (errRET <> 0) then
      call LOGERR(2)
    end if
    ...
    strRCMD = chr(34) & "c:\IT\" & strCID & "WindowsAgentSetup.exe" & chr(34) & " /s /v" & chr(34) & " /qn /norestart /l*v c:\temp\agent_install.log" & _

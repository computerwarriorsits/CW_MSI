...
if (errRET = 0) then                                        ''ARGUMENTS PASSED, CONTINUE SCRIPT
  ...
  if ((intRET = 4) or (intRET = 10) or (intRET = 11) or (intRET = 1) or (intRET = 2147221505) or (intRET = 2147221517)) then
    ...
    call FILEDL("https://raw.githubusercontent.com/computerwarriorsits/CW_MSI/master/Windows%20Agent.msi", "C:\IT", "windows agent.msi")
    if (errRET <> 0) then
      call LOGERR(2)
    end if
    ...
    strRCMD = "msiexec /i " & chr(34) & "c:\IT\windows agent.msi" & chr(34) & " /qn CUSTOMERID=" & strCID & _

...
if (errRET = 0) then                                        ''ARGUMENTS PASSED , CONTINUE SCRIPT
  ...
  if ((intRET = 4) or (intRET = 10) or (intRET = 11) or (intRET = 1) or (intRET = 2147221505) or (intRET = 2147221517)) then
    ...
    call FILEDL("https://raw.githubusercontent.com/computerwarriorsits/CW_MSI/master/WindowsProbeSetup.exe", "C:\IT", "WindowsSoftwareProbe.exe")
    if (errRET <> 0) then
      call LOGERR(4)
    end if
...
sub CLEANUP()                                               ''SCRIPT CLEANUP
  ...
  if (errRET = 0) then         															''EXE_REPROBE COMPLETED SUCCESSFULLY
    ...
    objLOG.write vbnewline & "EXE_REPROBE SUCCESSFUL : " & errRET & " : " & now
    err.clear
  elseif (errRET <> 0) then    															''EXE_REPROBE FAILED
    objOUT.write vbnewline & "EXE_REPROBE FAILURE : " & errRET & " : " & now
    objLOG.write vbnewline & "EXE_REPROBE FAILURE : " & errRET & " : " & now
